...
for i in range(nodes):
        ...
        if(j==0):
            for k, line in enumerate(fp1):
                if k>= 3:
                    ...
                    list1 = list1 * normweighting[j,k-4]
                    # print(k, list1)
                    normp[k,1:]= np.add(normp[k,1:],list1[1:])
    ...
    # print(normp[2003:])
    np.savetxt(EXDIR1+"/run-"+str(i+1)+"/normpop.out",normp[3:,:], delimiter=' ', header=  "Time Norm Re(ACF(t)) Im(ACF(t)) |ACF(t)| Re(Extra) Im(Extra) |Extra| Sum(HEhr) Pop1 Pop2 Pop1+Pop2 Pop2-Pop1")
