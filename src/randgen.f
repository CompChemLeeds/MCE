...
!      Present doesn't seem to be compatible with all compilers in F77 mode.
!      if (present(fu)) then
!         myfu = fu
!      else
!         myfu = 6
...
!       CLOSE(FILNO)
!       write(myfu,*) "sysdate", SS,MM,HH,DD

       call date_and_time(VALUES=values)

!       call idate(today(2), today(1), today(3))! today(2)=day, (1)=month, (3)=year
...
!       HH = now(1)
       ...
       ! The portland group compiler gives a number in the range [0:99]
!       if (today(3) < 100) then
!          today(3) = 2000+today(3)
!       end if
!       DD = yearday(today(2),today(1),today(3))
...
!! Present doesn't seem to be compatible with all compilers in F77 mode.
!!       if(present(fu)) then
!!          write(myfu,*) "  Seed from clock ",DSS+DMM+DHH+DDD
       SEED = dint(DSS+DMM+DHH+DDD)
       if(fu .ge. 0) then
!            Print *, 'Can you see me????'  !    written by JK
!          write(fu,*) "  Seed from clock ",DSS+DMM+DHH+DDD
